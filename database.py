...
class Database:
    ...
    # ===============================================
    # Operações para Drop de Trades
    # ===============================================

    def get_last_drop_time(self):
        """Obtém o timestamp do último drop de trade."""
        if not self.is_connected():
            return None
            
        try:
            result = self.db['drop_cooldowns'].find_one({'id': 'global_drop'})
            return result['timestamp'] if result else None
        except Exception as e:
            print(f"❌ Erro ao obter último drop: {e}")
            return None

    def set_last_drop_time(self, timestamp=None):
        """Define o timestamp do último drop de trade."""
        if not self.is_connected():
            return False
        ...
        try:
            self.db['drop_cooldowns'].update_one(
                {'id': 'global_drop'},
                {'$set': {'id': 'global_drop', 'timestamp': timestamp}},
                upsert=True
            )
            return True
        except Exception as e:
            print(f"❌ Erro ao definir último drop: {e}")
            return False

    def get_drop_collectors(self, drop_id):
        """Obtém os usuários que coletaram um drop específico."""
        if not self.is_connected():
            return []
            
        try:
            result = self.db['drop_collectors'].find_one({'drop_id': drop_id})
            return result['collectors'] if result and 'collectors' in result else []
        except Exception as e:
            print(f"❌ Erro ao obter coletores do drop {drop_id}: {e}")
            return []

    def add_drop_collector(self, drop_id, user_id):
        """Adiciona um usuário à lista de coletores de um drop."""
        if not self.is_connected():
            return False
            
        try:
            self.db['drop_collectors'].update_one(
                {'drop_id': drop_id},
                {'$addToSet': {'collectors': user_id}},
                upsert=True
            )
            return True
        except Exception as e:
            print(f"❌ Erro ao adicionar coletor ao drop {drop_id}: {e}")
            return False

    def has_collected_drop(self, drop_id, user_id):
        """Verifica se um usuário já coletou um drop específico."""
        if not self.is_connected():
            return False
            
        try:
            result = self.db['drop_collectors'].find_one(
                {'drop_id': drop_id, 'collectors': user_id}
            )
            return result is not None
        except Exception as e:
            print(f"❌ Erro ao verificar se usuário coletou drop: {e}")
            return False