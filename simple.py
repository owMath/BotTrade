...
@commands.has_role(GIVEAWAY_ROLE_ID)  # Apenas staff giveaway pode usar
async def guess_command(ctx, min_num: int, max_num: int, trades: int, *, prize: str = "Trades"):
    """Inicia um jogo de adivinhação de número"""
    # Obter idioma do usuário
    lang = get_user_language(ctx.author.id)

    # Verifica se o comando está sendo usado no canal correto
    if ctx.channel.id != 1158536057055346789:  # ID do canal específico
        await ctx.send(t('guess_wrong_channel', lang))
        return

    # Verifica se já existe um jogo ativo
    if ctx.channel.id in active_guess_games:
        await ctx.send(t('guess_game_active', lang))
        return

    # Reabre o canal para escrita
    await ctx.channel.set_permissions(ctx.guild.default_role, send_messages=True)

    # Gera um número aleatório entre min_num e max_num
    target_number = random.randint(min_num, max_num)
    
    # Envia o número para o staff via DM
    try:
        await ctx.author.send(f"O número para adivinhar é: {target_number}")
    except discord.Forbidden:
        await ctx.send(t('guess_dm_error', lang))
        return
    ...
    embed = discord.Embed(
        title=t('guess_title', lang),
        description=t('guess_desc', lang, {
            'min': min_num,
            'max': max_num,
            'trades': trades,
            'prize': prize
...
async def on_message(message):
    ...
    # Verifica se é uma mensagem no canal de guess
    if message.channel.id == 1158536057055346789 and message.channel.id in active_guess_games:
        ...
        try:
            ...
            game = active_guess_games[message.channel.id]

            # Verifica se o número está dentro do intervalo
            if guess < game['min'] or guess > game['max']:
                await message.channel.send(t('guess_range_error', lang, {
                    'min': game['min'],
                    'max': game['max']
                }))
                return
            ...
            if guess == game['target']:
                # Adiciona os trades ao ganhador
                if message.author.id not in user_trades:
                    user_trades[message.author.id] = 0
                user_trades[message.author.id] += game['trades']

                # Atualiza no banco de dados
                if db.is_connected():
                    db.increment_user_trades(message.author.id, game['trades'])

                # Cria embed de vitória
                embed = discord.Embed(
                    title=t('guess_win_title', lang),
                    description=t('guess_win_desc', lang, {
                ...
                # Fecha o canal para escrita
                await message.channel.set_permissions(message.guild.default_role, send_messages=False)

                # Remove o jogo ativo
                del active_guess_games[message.channel.id]

                # Envia DM para o ganhador
                try:
                    await message.author.send(t('guess_win_dm', lang, {
                        'trades': game['trades'],
                        'prize': game['prize'],
                        'number': game['target']
                    }))
                ...
                # Dá dica se o número é maior ou menor
                hint = "maior" if guess < game['target'] else "menor"
                await message.channel.send(t('guess_hint', lang, {
                    'hint': hint,
                    'guess': guess
                }))
