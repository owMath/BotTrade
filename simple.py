...
# ===============================================
# Classes para Componentes de UI
# ===============================================

# Classe para o botão de lembrete do slot
...
# View para o botão de lembrete do slot
...
# ===============================================
# Inicialização dos Comandos Slash
# ===============================================
...
async def on_ready():
    ...
    try:
        ...
        # Sincronizar comandos slash com o Discord (use apenas durante desenvolvimento)
        try:
...
# ===============================================
# Comandos Slash para usuários
# ===============================================

@bot.tree.command(name="listtrades", description="Verificar quantos trades você possui")
async def listtrades_slash(interaction: discord.Interaction):
    """Comando slash para verificar quantos trades o usuário possui"""
    try:
        # Verificar se está no canal correto
        if TRADE_CHANNEL_ID and str(interaction.channel_id) != str(TRADE_CHANNEL_ID).strip():
            await interaction.response.send_message(
                t('wrong_channel', get_user_language(interaction.user.id), 
                {'channel': f"<#{TRADE_CHANNEL_ID}>"}), 
                ephemeral=True
            )
            return
        ...
        if user_id not in user_trades or user_trades[user_id] <= 0:
            await interaction.response.send_message(t('no_trades_available', lang), ephemeral=True)
            return
        ...
        user_id = interaction.user.id
        lang = get_user_language(user_id)
        await log_error(f"Erro no comando slash listtrades: {e}")
        await interaction.response.send_message(t('command_error', lang), ephemeral=True)

@bot.tree.command(name="claimtrade", description="Obter trades diários (5 trades a cada 24 horas)")
async def claimtrade_slash(interaction: discord.Interaction):
    """Comando slash para usuários obterem trades diários"""
    try:
        # Verificar se está no canal correto
        if TRADE_CHANNEL_ID and str(interaction.channel_id) != str(TRADE_CHANNEL_ID).strip():
            await interaction.response.send_message(
        ...
        user_id = interaction.user.id
        lang = get_user_language(user_id)
        await log_error(f"Erro no comando slash claimtrade: {e}")
        await interaction.response.send_message(t('command_error', lang), ephemeral=True)

@bot.tree.command(name="usetrade", description="Usar um trade disponível com quantidade específica de trades")
@app_commands.describe(trades_amount="Quantidade de trades a processar (1-10)")
async def usetrade_slash(interaction: discord.Interaction, trades_amount: int = 2):
    """Comando slash para usuários usarem um trade disponível"""
    try:
        # Verificar se está no canal correto
        if TRADE_CHANNEL_ID and str(interaction.channel_id) != str(TRADE_CHANNEL_ID).strip():
            await interaction.response.send_message(
        ...
        if user_trades[user_id] < trades_amount:
            await interaction.response.send_message(t('not_enough_trades', lang, {'available': user_trades[user_id], 'requested': trades_amount}), ephemeral=True)
            return
        ...
        try:
            ...
            await process_trade_with_dm(interaction, code, dm_message, trades_amount)
            ...
            await interaction.followup.send(t('dm_blocked', lang, {'user': interaction.user.mention}))
            
            # Remover o trade ativo já que não podemos prosseguir
            if user_id in users_with_active_trade:
                del users_with_active_trade[user_id]
        ...
        user_id = interaction.user.id
        lang = get_user_language(user_id)
        await log_error(f"Erro no comando slash usetrade: {e}")
        await interaction.response.send_message(t('command_error', lang), ephemeral=True)

@bot.tree.command(name="slot", description="Jogar na slot machine para ganhar trades")
async def slot_slash(interaction: discord.Interaction):
    """Comando slash para jogar na slot machine"""
    try:
        # Verificar se está no canal correto
        if TRADE_CHANNEL_ID and str(interaction.channel_id) != str(TRADE_CHANNEL_ID).strip():
            await interaction.response.send_message(
        ...
        user_id = interaction.user.id
        lang = get_user_language(user_id)
        await log_error(f"Erro no comando slash slot: {e}")
        await interaction.response.send_message(t('command_error', lang), ephemeral=True)

@bot.tree.command(name="box", description="Jogar o jogo da caixa para ganhar trades")
async def box_slash(interaction: discord.Interaction):
    """Comando slash para jogar o jogo da caixa"""
    try:
        # Verificar se está no canal correto
        if TRADE_CHANNEL_ID and str(interaction.channel_id) != str(TRADE_CHANNEL_ID).strip():
            await interaction.response.send_message(
        ...
        user_id = interaction.user.id
        lang = get_user_language(user_id)
        await log_error(f"Erro no comando slash box: {e}")
        await interaction.response.send_message(t('command_error', lang), ephemeral=True)

@bot.tree.command(name="abort", description="Cancelar um trade ativo usando seu código")
@app_commands.describe(code="Código do trade a ser cancelado")
async def abort_slash(interaction: discord.Interaction, code: str):
    """Comando slash para cancelar um código de trade ativo"""
    try:
        user_id = interaction.user.id
        # Obter idioma do usuário
        lang = get_user_language(user_id)
        ...
        if not code:
            await interaction.response.send_message(t('abort_no_code', lang), ephemeral=True)
            return
        ...
        if code_info['user_id'] != user_id and not interaction.user.guild_permissions.administrator:
        ...
        # Se o usuário tem este código como ativo, remove do dicionário de usuários com trades ativos
        owner_id = code_info.get('user_id')
        if owner_id in users_with_active_trade and users_with_active_trade[owner_id] == code:
            del users_with_active_trade[owner_id]
            
            # Atualizar no MongoDB
            if db.is_connected():
                db.remove_user_active_trade(owner_id)
                db.delete_active_trade(code)
        ...
        user_id = interaction.user.id
        lang = get_user_language(user_id)
        await log_error(f"Erro no comando slash abort: {e}")
        await interaction.response.send_message(t('command_error', lang), ephemeral=True)

@bot.tree.command(name="help", description="Exibir ajuda sobre os comandos do bot")
async def help_slash(interaction: discord.Interaction):
    """Comando slash para exibir ajuda sobre os comandos do bot"""
    try:
        # Obter idioma do usuário
        lang = get_user_language(interaction.user.id)
        
        # Criar embed para usuários normais
        embed = discord.Embed(
        ...
        # Se for admin, mostrar embed com comandos de admin
        if interaction.user.guild_permissions.administrator:
            ...
            await interaction.response.send_message(embeds=[embed, admin_embed], ephemeral=False)
        else:
            await interaction.response.send_message(embed=embed, ephemeral=False)
    except Exception as e:
        user_id = interaction.user.id
        lang = get_user_language(user_id)
        await log_error(f"Erro no comando slash help: {e}")
        await interaction.response.send_message(t('command_error', lang), ephemeral=True)

@bot.tree.command(name="lang", description="Definir idioma preferido (pt, en, es)")
...
async def lang_slash(interaction: discord.Interaction, language_code: str = None):
    """Comando slash para definir o idioma preferido"""
    try:
        user_id = interaction.user.id
        ...
        if language_code not in ['pt', 'en', 'es']:
            ...
            current_lang = get_user_language(user_id)
            await interaction.response.send_message(t('invalid_language', current_lang, {'code': language_code}), ephemeral=True)
            return
        ...
        await interaction.response.send_message(t('command_error', current_lang), ephemeral=True)

# ===============================================
# Comandos Slash para administradores
# ===============================================

@bot.tree.command(name="trade", description="Iniciar trades automaticamente (modo por contagem)")
@app_commands.describe(
    trades_count="Quantidade de trades (1-10)",
    expire_minutes="Tempo de expiração em minutos (1-120)"
)
async def trade_slash(interaction: discord.Interaction, trades_count: int = 1, expire_minutes: int = 30):
    """Comando slash para iniciar trades automaticamente (modo por contagem)"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
            return
        ...
        # Confirmar que os trades estão sendo gerados
        await interaction.response.send_message(t('trade_code_generated', lang, {'code': '...', 'minutes': expire_minutes}))
        ...
        for _ in range(trades_count):
            ...
            # Preparar tarefa para processar o trade
            task = asyncio.create_task(process_trade_slash(interaction, code))
            tasks.append(task)
        
        # Atualizar a mensagem inicial com os códigos gerados
        code_list = "\n".join([f"- `{code}`" for code in trade_codes])
        await interaction.followup.send(f"Códigos gerados (validade: {expire_minutes} minutos):\n{code_list}")
        
        # Esperar a conclusão de todas as tarefas
        await asyncio.gather(*tasks)
    except Exception as e:
        await log_error(f"Erro no comando trade slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
            await interaction.response.send_message(t('command_error', lang), ephemeral=True)

@bot.tree.command(name="timemode", description="Iniciar trades em modo tempo (processando por X minutos)")
@app_commands.describe(
    duration="Duração do processamento em minutos (1-120)",
    expire_minutes="Tempo de expiração em minutos (1-120)"
)
async def timemode_slash(interaction: discord.Interaction, duration: int = 30, expire_minutes: int = 30):
    """Comando slash para iniciar trades em modo tempo"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
        ...
        await interaction.response.send_message(t('trade_time_mode', lang, {'code': code, 'duration': duration, 'minutes': expire_minutes}))
        
        # Processar o trade em modo tempo
        await process_trade_slash(interaction, code)
    except Exception as e:
        await log_error(f"Erro no comando timemode slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
@bot.tree.command(name="givetrade", description="Dar trades a um usuário")
@app_commands.describe(
    member="Usuário que receberá os trades",
    amount="Quantidade de trades (1-100)"
)
async def givetrade_slash(interaction: discord.Interaction, member: discord.Member, amount: int = 1):
    """Comando slash para administradores darem trades a um usuário"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
        ...
        if not member:
            await interaction.response.send_message(t('member_not_found', lang), ephemeral=True)
            return
        ...
        await log_error(f"Erro no comando givetrade slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
@bot.tree.command(name="status", description="Verificar o status de um código ou todos os seus códigos ativos")
@app_commands.describe(code="Código do trade (opcional)")
async def status_slash(interaction: discord.Interaction, code: str = None):
    """Comando slash para verificar o status de um código ou todos os seus códigos ativos"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
        ...
        await log_error(f"Erro no comando status slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
@bot.tree.command(name="activecodes", description="Mostrar todos os códigos ativos no sistema")
async def activecodes_slash(interaction: discord.Interaction):
    """Comando slash para administradores verem todos os códigos ativos no sistema"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
        ...
        await log_error(f"Erro no comando activecodes slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
@bot.tree.command(name="tradeshistory", description="Ver histórico de trades de um usuário")
@app_commands.describe(member="Usuário para ver o histórico (opcional, padrão: você mesmo)")
async def tradeshistory_slash(interaction: discord.Interaction, member: discord.Member = None):
    """Comando slash para ver o histórico de trades de um usuário"""
    try:
        # Obter idioma do usuário
        lang = get_user_language(interaction.user.id)
        ...
        await log_error(f"Erro no comando tradeshistory slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
@bot.tree.command(name="resetclaim", description="Resetar o cooldown de claim diário de um usuário")
...
async def resetclaim_slash(interaction: discord.Interaction, member: discord.Member):
    """Comando slash para resetar o cooldown de claim diário de um usuário"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
        ...
        if not member:
            await interaction.response.send_message(t('resetclaim_no_member', lang), ephemeral=True)
            return
        ...
        await log_error(f"Erro no comando resetclaim slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
@bot.tree.command(name="resetslot", description="Resetar o cooldown de slot de um usuário")
@app_commands.describe(member="Usuário para resetar o cooldown")
async def resetslot_slash(interaction: discord.Interaction, member: discord.Member):
    """Comando slash para resetar o cooldown de slot de um usuário"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
        ...
        # Verificar se o membro foi especificado
        if not member:
            await interaction.response.send_message(t('resetslot_no_member', lang), ephemeral=True)
            return
        ...
        await log_error(f"Erro no comando resetslot slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
@bot.tree.command(name="resetbox", description="Resetar o cooldown de box de um usuário")
@app_commands.describe(member="Usuário para resetar o cooldown")
async def resetbox_slash(interaction: discord.Interaction, member: discord.Member):
    """Comando slash para resetar o cooldown de box de um usuário"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
        ...
        # Verificar se o membro foi especificado
        if not member:
            await interaction.response.send_message(t('resetbox_no_member', lang), ephemeral=True)
            return
        ...
        await log_error(f"Erro no comando resetbox slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
@bot.tree.command(name="stats", description="Ver estatísticas de trades no sistema")
@app_commands.describe(period="Período (all, today, week, month)")
...
async def stats_slash(interaction: discord.Interaction, period: str = "all"):
    """Comando slash para ver estatísticas de trades no sistema"""
    try:
        # Verificar se o usuário é administrador
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(t('admin_only', get_user_language(interaction.user.id)), ephemeral=True)
        ...
        await log_error(f"Erro no comando stats slash: {e}")
        lang = get_user_language(interaction.user.id)
        if interaction.response.is_done():
            await interaction.followup.send(t('command_error', lang), ephemeral=True)
        else:
...
# ===============================================
# Funções auxiliares para comandos slash
# ===============================================

async def process_trade_slash(interaction, code):
    """Versão modificada da função process_trade para lidar com interações do slash command"""
    try:
        ...
        async with trade_semaphore:
            try:
                # Verificar modo de trade (time ou trades)
                mode = code_info.get('mode', 'trades')
                
                if mode == 'time':
                    # Modo tempo - executar por duração específica
                    duration = code_info.get('duration', 30)
                    ...
                    # Modo trades - executar quantidade específica de trades
                    trades_amount = code_info.get('trades_amount', 2)
                    returncode, stdout, stderr = await run_trade_process(
                        code, 
                        expire_minutes,
                        'trades',
                ...
                if returncode == 0:
                    ...
                    # Se a mensagem de interação já tiver sido respondida, usar followup
                    if interaction.response.is_done():
                        await interaction.followup.send(f"✅ Trade com código **{code}** processado com sucesso!")
                    
                    # Disparar evento de trade completado
                    bot.dispatch('trade_completed', interaction.user.id, code)
                    ...
                    # Se a mensagem de interação já tiver sido respondida, usar followup
                    if interaction.response.is_done():
                        await interaction.followup.send(f"❌ Falha ao processar trade com código **{code}**")
                ...
                # Se a mensagem de interação já tiver sido respondida, usar followup
                if interaction.response.is_done():
                    await interaction.followup.send(f"❌ Erro ao processar trade: {str(e)}")
    except Exception as e:
        await log_error(f"Erro em process_trade_slash: {e}")

async def process_trade_with_dm(interaction, code, dm_message, trades_amount):
    """Função para processar um trade com atualizações via DM para comandos slash"""
    try:
        # Obter idioma do usuário
        lang = get_user_language(interaction.user.id)
        ...
        async with trade_semaphore:
            try:
                ...
                if returncode == 0:
                    ...
                    if interaction.response.is_done():
                        await interaction.followup.send(t('trade_error_public', lang, {'mention': interaction.user.mention}))
        ...
        await log_error(f"Erro em process_trade_with_dm para slash command: {e}")
        try:
            await dm_message.edit(content=f"❌ Erro ao processar trade: {str(e)}")
        except:
            pass
...
# ===============================================
# Manter compatibilidade com comandos originais
# ===============================================

# Aqui você pode manter todas as suas funções originais com o prefixo "!"
